...
def get_slider_info(ws):
    ...
    js = """(() => {
        ...
        return JSON.stringify({
            ...
            bgImg: bRect ? {x: bRect.x, y: bRect.y, w: bRect.width, h: bRect.height, src: bgImg.src} : null,
            jigsaw: jRect ? {x: jRect.x, y: jRect.y, w: jRect.width, h: jRect.height, src: jigsaw.src} : null,
...
def solve_slider(ws, slider_info, max_retries=4):
    ...
    for attempt in range(max_retries):
        ...
        # 下载背景图分析缺口
        gap_x = None
        if bg_info and bg_info.get("src"):
            try:
...
def main():
    ...
    for _ in range(10):
        ...
        if bg and bg.get("w", 0) > 50 and jig and jig.get("w", 0) > 10:
            print("  验证码图片已加载")
            break
        time.sleep(0.5)
        slider_info = get_slider_info(ws)
    ...
    print("  滑块轨道: x=%.0f y=%.0f w=%.0f h=%.0f" % (cr["x"], cr["y"], cr["w"], cr["h"]))
    bg = slider_info.get("bgImg")
    if bg:
        print("  背景图: x=%.0f y=%.0f w=%.0f h=%.0f" % (bg["x"], bg["y"], bg["w"], bg["h"]))

    success = solve_slider(ws, slider_info, max_retries=4)
